...
contract ArtMarketplace is IERC721Receiver {
    ...
    modifier OnlyItemOwner(uint256 tokenId) {
        require(
            artCollection.ownerOf(tokenId) == address(this),
    ...
    modifier ItemExists(uint256 tokenId) {
        require(
            itemsForSale[tokenId].seller != address(0x0),
    ...
    function _putItemForSale(uint256 tokenId) private OnlyItemOwner(tokenId) {
    ...
    {
        require(
            msg.value >= itemsForSale[tokenId].price,
    ...
    function mint(uint256 price, string memory uri) public returns (uint256) {
